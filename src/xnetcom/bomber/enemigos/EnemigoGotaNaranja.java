...
public class EnemigoGotaNaranja extends EnemigoBase {
	...
	public EnemigoGotaNaranja(BomberGame context) {
		super(context);
	}
...
//				Log.d("UPDATE", "GLOBO baseTileRectangle");
	...
	public void attachSpriteGroup() {
		...
		if (spritePrincipal != null) {
			...
			spritePrincipal = new AnimatedSprite(0, 0, context.almacenEnemigos.gotaNaranjaTR, context.getVertexBufferObjectManager());
			...
			context.almacenEnemigos.groupGotaNaranja.attachChild(spritePrincipal);
	...
	private void jump(Direction dirSeguidorSalto) {
		...
		if (dirSeguidorSalto==Direction.DOWN || dirSeguidorSalto==Direction.UP){
			...
			SequenceEntityModifier jumpModifier=new SequenceEntityModifier(new ScaleModifier(jumpDuration, 1, 1.3f,EaseExponentialOut.getInstance()),new ScaleModifier(jumpDuration, 1.3f, 1f,EaseBounceOut.getInstance()));
