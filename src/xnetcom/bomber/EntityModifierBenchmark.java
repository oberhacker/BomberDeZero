 ...
 */
public class EntityModifierBenchmark extends SimpleBaseGameActivity {
	// ===========================================================
	// Constants
	// ===========================================================
...
//		this.drawUsingSpritesWithSharedVertexBuffer(scene);
	...
	private void drawUsingSprites(final Scene pScene) {
		...
		for(int i = 0; i < SPRITE_COUNT; i++) {
			final Sprite face = new Sprite((CAMERA_WIDTH - 32) * new Random(23424234).nextFloat(), (CAMERA_HEIGHT - 32) * new Random(23424234).nextFloat(), this.mFaceTextureRegion, vertexBufferObjectManager);
	...
	private void drawUsingSpritesWithSharedVertexBuffer(final Scene pScene) {
		...
		for(int i = 0; i < SPRITE_COUNT; i++) {
			final Sprite face = new Sprite((CAMERA_WIDTH - 32) * new Random(23424234).nextFloat(), (CAMERA_HEIGHT - 32) * new Random(23424234).nextFloat(), this.mFaceTextureRegion, sharedVertexBuffer);
			face.setBlendFunction(GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA);
			face.registerEntityModifier(faceEntityModifier.deepCopy());
	...
	private void drawUsingSpriteBatch(final Scene pScene) {
		...
		for(int i = 0; i < EntityModifierBenchmark.SPRITE_COUNT; i++) {
			final Sprite face = new Sprite((EntityModifierBenchmark.CAMERA_WIDTH - 32) * new Random(23424234).nextFloat(), (EntityModifierBenchmark.CAMERA_HEIGHT - 32) * new Random(23424234).nextFloat(), this.mFaceTextureRegion, vertexBufferObjectManager);
			face.registerEntityModifier(faceEntityModifier.deepCopy());
